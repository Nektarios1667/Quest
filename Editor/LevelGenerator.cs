    ...
    public Tile[] GenerateTerrain(int width, int height)
    {
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                level[y * width + x] = Tile.TileFromId(tileType, new(x, y));
            }
        }
        ...
        for (int i = 0; i < structureAttempts; i++)
        {
            ...
            for (int y = 0; y < structure.Size.Y; y++)
            {
                for (int x = 0; x < structure.Size.X; x++)
                {
                    ...
                    level[(spawnPoint.Y + y) * width + spawnPoint.X + x] = Tile.TileFromId(tileType.Value, new(spawnPoint.X + x, spawnPoint.Y + y));
                }
            }
