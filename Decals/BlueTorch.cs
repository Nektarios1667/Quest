...
public class BlueTorch(Point location) : Decal(location)
{
    ...
    public override void Draw(GameManager game)
    {
        ...
        LightingManager.SetLight($"BlueTorchDecal_{Location.X}_{Location.Y}", (Location.ToVector2() + lightShift).ToPoint() * Constants.TileSize - CameraManager.Camera.ToPoint() + Constants.Middle, 5, new(0, 50, 50, 60));
