...
public class Torch(Point location) : Decal(location)
{
    ...
    public override void Draw(GameManager game)
    {
        ...
        LightingManager.SetLight($"TorchDecal_{Location.X}_{Location.Y}", (Location.ToVector2() + lightShift).ToPoint() * Constants.TileSize - CameraManager.Camera.ToPoint() + Constants.Middle, 6, new(30, 8, 0, 200));
