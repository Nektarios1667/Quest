...
public class Torch(Point location) : Decal(location)
{
    public override void Draw(GameManager game)
    {
        ...
        TextureManager.DrawTexture(game.Batch, TextureManager.TextureID.Glow, dest, scale: 4, color: Color.Lerp(Tint, Color.DarkOrange, .8f) * ((float)Math.Cos(game.TotalTime) / 8 + .4f));
        base.Draw(game);
    }
}
