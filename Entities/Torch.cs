...
public class Torch(Point location) : Decal(location)
{
    public override void Draw(IGameManager game)
    {
        ...
        DrawTexture(game.Batch, TextureID.Glow, new(dest, TextureManager.Metadata[TextureID.Glow].Size), scale: new(4), color: Color.Lerp(Tint, Color.DarkOrange, .8f) * ((float)Math.Cos(game.Time) / 8 + .4f));
        base.Draw(game);
    }
}
