...
public class OverlayManager
{
    ...
    public NotificationArea PlayerNotificationArea { get; private set; } // Loot pickup notifications
    ...
    public OverlayManager(PlayerManager playerManager)
    {
        ...
        Gui.Widgets = [
            ...
            PlayerNotificationArea = new NotificationArea(Constants.Middle - new Point(0, Constants.MageHalfSize.Y + 15), 5, PixelOperatorBold),
    ...
    public void Update(GameManager gameManager, PlayerManager? playerManager)
    {
        ...
        if (playerManager?.HoveredItem != null && playerManager.InventoryOpen)
        {
            ...
            ItemInfobox.SetText($"--- {StringTools.FillCamelSpaces(item.Name)} ---\n{item.Description}", respeak: DialogRespeak.Instant);
        }
        else
    ...
    public void DrawPostProcessing(GameManager gameManager, PlayerManager? playerManager)
    {
        ...
        // Guis
        if (gameManager.StateManager.OverlayState == OverlayState.Interface || gameManager.StateManager.OverlayState == OverlayState.Pause || gameManager.StateManager.OverlayState == OverlayState.GUI)
    ...
    {
        PlayerNotificationArea.AddNotification(text, color, duration);
    }
